...
int main()
{
    ...
    while (true)
    {
        ...
        int mode=0;//creates some integers used later in code
        int l=0;
        Timer flashTimer;//creates and starts a timer
        flashTimer.start();

        while(true){
            microseconds tflash = flashTimer.elapsed_time();
//            while(switchA==0&&switchB==0&&switchC==0&&switchD==0&&blueButton==0&&tflash<=1000ms);
            printf("mode: %d\n",mode);
            wait_us(1000);
            if(mode==1){
                grid.enableLEDS(1);
            }
            else if(tflash>=500ms){
                if(mode==0){
                    grid.enableLEDS(l=!l);
                }
                flashTimer.reset();
            ...
            grid.xyplot(x, y);
            display.cls();
            display.printf("(x,y)=(%u,%u)", x,y);
            while(switchA||switchB||switchC||switchD||blueButton);
        }
        printf("ree\n");
    }
}