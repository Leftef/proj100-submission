...
int main()
{
    ...
    waitTimer.start();
    while(true){
        while(blueButton==0&&waitTimer.elapsed_time()<1000ms);
        if(blueButton){// (c) If the blue button is pressed during the countdown: stop the countdown, wait for the button to be released and restart the program.
            while(blueButton);
            main();
        }
        if(waitTimer.elapsed_time()>=1000ms){
            led1=!led1;
            counter=counter-1;
            if(~counter){
                display.cls();
...
if((swA+swB+swC+swD)==1){
    ...
    if(swD){
        for(int X=0;X<15;X++){
            wait_us(1000000);
            ledTraffic1Red=!ledTraffic1Red;
            ledTraffic1Green=!ledTraffic1Green;
            ledTraffic1Yellow=!ledTraffic1Yellow;
        }
    }
}
else if((swA+swB+swC+swD)>1){
    ledTraffic1Red=1;
    wait_us(1000000);
    ledTraffic1Red=0;
    ledTraffic1Yellow=1;
